...
class MeRGAN(object):
    def __init__(self, dataset, class_num, epoch, batch_size, num_generated, result_dir, method):
        # dataset == MNIST 면 input_dim=100, total_class_num=10
        # ACGAN에 result_dir 전달할 때는 result_dir + '/' + dataset으로

        self.total_class_num = class_num
        self.epoch = epoch
        self.batch_size = batch_size
        self.num_in_class = num_generated
        self.method = method
        self.result_dir = result_dir + '/' + method
        self.class_array = []
        self.data_list = []
        if dataset == 'MNIST':
    ...
    def init_ACGAN(self, dataset, class_index):
        ...
        self.ACGAN = ACGAN(data_loader=torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, shuffle=True),
                           class_index=len(self.class_array) + 1, epoch=self.epoch)
        self.ACGAN.train()
